...
# rrr_cat_file = "../test/rapid_catchment_82.csv"
# rrr_lsm_file = "../test/GLDAS_v21_2015_2024_ens.nc"
# rrr_cpl_file = "../test/cpl_82.csv"

# python rrr_cpl_riv_lsm_lnk.py \
#         /Users/elyssac/Documents/RiverRouting/inputs/b82/Main_Side/con_82.csv \
#         /Users/elyssac/Documents/RiverRouting/inputs/b82/Main_Side/rapid_catchment_82.csv \
...
with open(rrr_cat_file) as csv_file:
     ...
     for row in reader:
          ...
          ZV_cat_sqkm.append(float(row[1]))
          ZV_cat_lon.append(float(row[2]))
          ZV_cat_lat.append(float(row[3]))
...
for JS_lsm_lat in range(IS_lsm_lat):
     ...
     ZV_lsm_lat.append(numpy.round(float(f.variables['lat'][JS_lsm_lat]),2))

ZV_lsm_lon=[]
for JS_lsm_lon in range(IS_lsm_lon):
     #ZV_lsm_lon.append(f.variables['lon'][JS_lsm_lon])
     #Rounding allows reproducing results from ArcGIS and speeds up comparisons:
     ZV_lsm_lon.append(numpy.round(float(f.variables['lon'][JS_lsm_lon]),2))
